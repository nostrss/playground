...
export default function DarkModeToggle() {
  ...
  const initializeDarkMode = () => {
    ...
    const prefersDarkMode = window.matchMedia(`(prefers-color-scheme: ${DARK_MODE})`).matches
  ...
  const chageModeInvert = () => {
    const currentMode = document.querySelector('meta[name="color-scheme"]')?.getAttribute('content')
  ...
  return (
    <>
      <button onClick={chageModeInvert} title={isDark ? '일반모드로 변경' : '다크모드로 변경'}>
